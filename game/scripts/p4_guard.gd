...
func _rebuild_fan() -> void:
	...
	# at current animated target heights. Actual detection always uses the
	# exact head/chest rays, never this mesh or a 2D footprint.
	var target_heights: Array[float] = [1.62, 1.20]
	if is_instance_valid(_world.player):
		target_heights.clear()
		for point in _world.player.get_detection_points():
			target_heights.append(point.y - _world.player.global_position.y)
	var eye := eye_position()
	...
	for angle_index in ANGLES:
		...
		for ring in RINGS:
			...
			for height in target_heights:
				var target := Vector3(point.x, global_position.y + height, point.z)
				if _point_in_view(target, eye) and _ray_clear(eye, target):
					seen = true
					break
			if not seen:
				continue
