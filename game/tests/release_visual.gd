...
class StagedWorld extends "res://scripts/release.gd":
...
func _run() -> void:
	...
	file.store_string(JSON.stringify({"purpose": "Staged native release full-level render inspection", "captures": captures}, "\t"))
	print("RELEASE_VISUAL_OK screenshots=%d" % captures.size())
	world.queue_free()
	await process_frame
	quit()
